...
int main() {
  ...
  if (firstInfile.is_open()) {
    ...
    while (firstInfile >> wordStrings) {
      ...
      transform(wordStrings.begin(), wordStrings.end(), wordStrings.begin(),
                ::toupper);
      listOfWords.push_back(wordStrings);
    }
  }
  // -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -=
  ...
  if (secondInfile.is_open()) {
    while (secondInfile >> inputWord) {
      ...
      for (const auto &word : foundWordTwice) {
        if (word == inputWord) {
          inputWordFound = true;
          break;
        }
      ...
      if (!inputWordFound) {
        foundWordTwice.push_back(inputWord);
      }
  ...
  while (it != wordsToCheck.end()) {
    ...
    if (!wordFound) {
      ...
      vector<string> closestWords = findClosestWords(word);
      for (int i = 0; i < closestWords.size(); i++) {
        string &possWord = closestWords[i];
        if (count(listOfWords.begin(), listOfWords.end(), possWord)) {
          possibleWordSuggestion.push_back(possWord);
        }
      }
...
vector<string> findClosestWords(const string &inputtedWords) {
  ...
  // our vector string of FoundWord, which is a string
  ...
  for (int i = 0; i <= inputtedWords.length(); ++i) {
    ...
    for (char currentChar = 'A'; currentChar <= 'Z'; ++currentChar) {
      ...
      bool secondWFound = false;
      for (const auto &word : foundWord) {
        if (word == secondW) {
          secondWFound = true;
          break;
        }
      ...
      if (secondWFound ==
          false) { // inserts this at the end of the find word vector
